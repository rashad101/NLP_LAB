...
class RD(object):
    ...
    def get_weight_file_path(model_dir_path):
        return os.path.join(model_dir_path, RD.model_name + '-weights.h5')

    @staticmethod
    def get_architecture_file_path(model_dir_path):
        return os.path.join(model_dir_path, RD.model_name + '-architecture.json')
    ...
    def load_model(self, model_dir_path):
        self.target_word2idx = np.load(
            model_dir_path + '/' + RD.model_name + '-target-word2idx.npy').item()
        self.target_idx2word = np.load(
            model_dir_path + '/' + RD.model_name + '-target-idx2word.npy').item()
        context = np.load(model_dir_path + '/' + RD.model_name + '-config.npy').item()
        ...
        self.create_model()
        self.model.load_weights(RD.get_weight_file_path(model_dir_path))
    ...
    def fit(self, data_set, model_dir_path, epochs=None, batch_size=None, test_size=None, random_state=None,
        ...
        data_set_seq2seq = LCQuADSeq2SeqEmbTripleSamples(data_set, self.glove_model.word2em,
                                                        self.glove_model.embedding_size,
                                                        max_target_vocab_size=max_target_vocab_size)
        data_set_seq2seq.save(model_dir_path, '-glove')
        ...
        np.save(os.path.join(model_dir_path, RD.model_name + '-history.npy'), history.history)
